...
import com.savemaster.savefromfb.player.helper.LockManager;

import static com.savemaster.savefromfb.BuildConfig.APPLICATION_ID;
...
public class DownloadManagerService extends Service {
	...
	public void onCreate() {
		...
		Intent openDownloadListIntent = new Intent(this, UIDownloadActivity.class)
		...
		Builder builder = new Builder(this, getString(R.string.savemasterdown_notification_channel_id))
				.setContentIntent(mOpenDownloadList)
				.setSmallIcon(android.R.drawable.stat_sys_download)
				.setLargeIcon(icLauncher)
				.setContentTitle(getString(R.string.savemasterdown_msg_running))
				.setContentText(getString(R.string.savemasterdown_msg_running_detail));
		...
		handlePreferenceChange(mPrefs, getString(R.string.savemasterdown_downloads_cross_network));
		handlePreferenceChange(mPrefs, getString(R.string.savemasterdown_downloads_maximum_retry));
		...
		if (action != null) {
			if (action.equals(Intent.ACTION_RUN)) {
				...
				if (action.equals(ACTION_OPEN_DOWNLOADS_FINISHED)) {
					startActivity(new Intent(this, UIDownloadActivity.class)
										  .setAction(Intent.ACTION_MAIN)
	...
	private void handleConnectivityState(boolean updateOnly) {
		...
		DownloadManager.NetworkState status;
		...
		if (info == null) {
			...
			if (connected)
				status = metered ? DownloadManager.NetworkState.MeteredOperating : DownloadManager.NetworkState.Operating;
			else
				status = DownloadManager.NetworkState.Unavailable;
	...
	private void handlePreferenceChange(SharedPreferences prefs, @NonNull String key) {
		if (key.equals(getString(R.string.savemasterdown_downloads_maximum_retry))) {
			try {
				String value = prefs.getString(key, getString(R.string.savemasterdown_downloads_maximum_retry_default));
		...
		else if (key.equals(getString(R.string.savemasterdown_downloads_cross_network))) {
	...
	public void notifyFinishedDownload(String name) {
		...
		if (downloadDoneNotification == null) {
			...
			downloadDoneNotification = new Builder(this, getString(R.string.savemasterdown_notification_channel_id))
					.setAutoCancel(true)
	...
	public void notifyFailedDownload(DownloadMission mission) {
		...
		if (downloadFailedNotification == null) {
			...
			downloadFailedNotification = new Builder(this, getString(R.string.savemasterdown_notification_channel_id))
					.setAutoCancel(true)

...
public class CustomTrackSelector extends DefaultTrackSelector {
	...
	@Override
	@Nullable
	protected Pair<TrackSelection.Definition, TextTrackScore> selectTextTrack(final TrackGroupArray groups, @NonNull final int[][] formatSupport, @NonNull final Parameters params, @Nullable final String selectedAudioLanguage) {
		...
		for (int groupIndex = 0; groupIndex < groups.length; groupIndex++) {
			...
			for (int trackIndex = 0; trackIndex < trackGroup.length; trackIndex++) {
				if (isSupported(trackFormatSupport[trackIndex], params.exceedRendererCapabilitiesIfNecessary)) {
					...
					else if (trackScore.isWithinConstraints && (selectedTrackScore == null || trackScore.compareTo(selectedTrackScore) > 0)) {
						selectedGroup = trackGroup;
						selectedTrackIndex = trackIndex;
						selectedTrackScore = trackScore;
					}

...
public class LoadedMediaSource implements ManagedMediaSource {
	...
	public LoadedMediaSource(@NonNull final MediaSource source, @NonNull final PlayQueueItem stream, final long expireTimestamp) {
		this.source = source;
		this.stream = stream;
	...
	@Override
	public void prepareSource(final MediaSourceCaller mediaSourceCaller, @Nullable final TransferListener mediaTransferListener) {
	...
	@Override
	public MediaPeriod createPeriod(final MediaPeriodId id, final Allocator allocator, final long startPositionUs) {
	...
	@Override
	public void addEventListener(final Handler handler, final MediaSourceEventListener eventListener) {
	...
	@Override
	public boolean shouldBeReplacedWith(@NonNull final PlayQueueItem newIdentity, final boolean isInterruptable) {

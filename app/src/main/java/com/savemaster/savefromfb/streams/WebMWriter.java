...
public class WebMWriter implements Closeable {
	...
	public WebMWriter(SharpStream... source) {
		...
		infoTracks = new WebMReader.WebMTrack[sourceTracks.length];
		outBuffer = new byte[BUFFER_SIZE];
		outByteBuffer = ByteBuffer.wrap(outBuffer);
		clustersOffsetsSizes = new ArrayList<>(256);
	}
	
	public WebMReader.WebMTrack[] getTracksFromSource(int sourceIndex) throws IllegalStateException {
		if (done) {
	...
	public void selectTracks(int... trackIndex) throws IOException {
		try {
			readersSegment = new WebMReader.Segment[readers.length];
			readersCluster = new WebMReader.Cluster[readers.length];
	...
	private Block getNextBlockFrom(int internalTrackId) throws IOException {
		...
		WebMReader.SimpleBlock res = readersCluster[internalTrackId].getNextSimpleBlock();
		if (res == null) {
	...
	private ArrayList<byte[]> makeTrackEntry(int internalTrackId, WebMReader.WebMTrack track) {

...
public class Mp4FromDashWriter {
	...
	public Mp4DashReader.Mp4Track[] getTracksFromSource(int sourceIndex) throws IllegalStateException {
		if (!parsed) {
	...
	public void build(SharpStream output) throws IOException {
		...
		while (written > 0) {
			...
			for (int i = 0; i < readers.length; i++) {
				...
				int j = 0;
				for (; j < limit; j++) {
					Mp4DashReader.Mp4DashSample sample = getNextSample(i);
	...
	private Mp4DashReader.Mp4DashSample getNextSample(int track) throws IOException {
		...
		Mp4DashReader.Mp4DashSample sample = readersChunks[track].getNextSample();
		if (sample == null) {
	...
	private void make_trak(int index, long duration, int defaultMediaTime, TablesInfo tables, boolean is64) throws IOException {
		...
		make_mdia(tracks[index].trak.mdia, tables, is64, tracks[index].kind == Mp4DashReader.TrackKind.Audio);
		
		lengthFor(start);
	}
	
	private void make_mdia(Mp4DashReader.Mdia mdia, TablesInfo tablesInfo, boolean is64, boolean isAudio) throws IOException {

...
public class QualityBottomsheetAdapter extends RecyclerView.Adapter<QualityBottomsheetAdapter.ViewHolder> {
    ...
    public QualityBottomsheetAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.savemasterdown_downl_oadite_rament, null, false);
...
//                    DownloadFileMain.startDownloading(context, vidurl, vidSource + "_" + System.currentTimeMillis(), ".mp4");
                    Downloader.getInstance(context).downloadByUrl(vidurl, vidSource + "_" + System.currentTimeMillis(), ".mp4");
                                    ...
                                    new View.OnClickListener() {
                                        ...
                                        public void onClick(View v) {
                                            if (files.getEXT().equals("com") || files.getEXT().equals("")) {
                                                Downloader.getInstance(context).downloadByUrl(files.getURL(), vidSource + "_" + System.currentTimeMillis(), ".mp4");
...
//                                                DownloadFileMain.startDownloading(context, files.getURL(), vidSource + "_" + files.getTitle(), "." + files.getEXT());
                                                ...
                                                Downloader.getInstance(context).downloadByUrl(files.getURL(), vidSource + "_" + System.currentTimeMillis(), "." + files.getEXT());
                                            } else {
//                                                DownloadFileMain.startDownloading(context, files.getURL(), vidSource + "_" + files.getTitle(), "." + files.getEXT());
...
//                    if (filesList.size() > 1) {
                ...
                if (!files.getURL().contains(".m3u8") && files.getProtocol().contains("http")) {
                    ...
                    String formatedsizew = MyUtils.getStringSizeLengthFile(files.getFilesize());
